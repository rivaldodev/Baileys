...
const makeInMemoryStore = (config) => {
    ...
    const bind = (ev) => {
        ...
        // Handle groups.upsert (new in latest Baileys)
        ...
        // Updated: participants are now GroupParticipant[] objects (with .id, .isAdmin, etc.)
        // instead of string[] in the latest Baileys
                ...
                switch (action) {
                    ...
                    case 'promote':
                        for (const participant of metadata.participants) {
                            const participantIds = participants.map(p => typeof p === 'string' ? p : (p.id || p.phoneNumber))
    ...
    return {
         ...
         * This can be used as the `getMessage` option in makeWASocket
         */
